...
def evaluate(vqg, data_loader, criterion, l2_criterion, args):
    ...
    for iterations, (images, questions, answers,
        ...
        total_gen_loss += gen_loss.data.item() #生成句子的损失
...
def run_eval(vqg, data_loader, criterion, l2_criterion, args, epoch,
    ...
    #返回两个重建损失的平均值，第一个是句子重建的MSE，第二个是answer和image的重建L2损失之和
    delta_time = time.time() - start_time
    scheduler.step(val_gen_loss)
    info_scheduler.step(val_info_loss)
...
def compare_outputs(images, questions, answers, categories,
                    ...
                    args, num_show=5):
    """Sanity check generated output as we train,during every validation time.
    Notice that we generate question from category and answer
    ...
    for _ in range(num_show):
        ...
        logging.info('Sampled question : %s\n'
                     ...
                     % (output, cat2name[categories[i].item()],
                        question, answer)) #这里我觉得最好把图片的id的信息也输出来，方便查看原本的图片。
        logging.info("         ")
...
def compute_two_gaussian_loss(mu1, logvar1, mu2, logvar2):
    ...
    fraction = torch.div(numerator, (logvar2.exp() + 1e-8)) #逐个元素运算
...
def train(args):
    ...
    info_optimizer = torch.optim.Adam(info_params, lr=info_learning_rate)#,weight_decay=0.01
    ...
    for epoch in range(args.num_epochs):
        for i, (images, questions, answers,
                categories, qindices) in enumerate(data_loader):
            n_steps += 1
    #images:128,3,224,224;数值已经处理过，大部分在【-2，2】之间
    #questions:128，20，注意，这里都是token的下标，还没有embedding
    #answers:128，4，注意，这里都是token的下标，还没有embedding
    #categories:128，也都是问题类别的下标
    #qindices:128，返回的是question对应的image的下标
            ...
            # Question generation.
            mus, logvars = vqg.encode_into_z(image_features, answer_features) 
            zs = vqg.reparameterize(mus, logvars) #VAE的采样，实际上一开始，mus都是0.几，logvar也在0附近的

            #从Z空间，也就是answer和image的VAE的隐空间采样，并生成一句话
            ...
            outputs = pack_padded_sequence(outputs, qlengths,
                                           batch_first=True)[0]
            gen_loss = criterion(outputs, targets) #计算语句的交叉熵，MLE
            total_loss = 0.0
            total_loss += args.lambda_gen * gen_loss
            gen_loss = gen_loss.item()

            # Variational loss.
            kl_loss = gaussian_KL_loss(mus, logvars) #这个似乎是，一般的VAEloss?对于z空间的VAE损失，最早25左右
            ...
            if not args.no_category_space:
                category_features = vqg.encode_categories(categories)
                t_mus, t_logvars = vqg.encode_into_t(
                        image_features, category_features)#这里的logvars是方差平方的对数，因为这个取值是Rn，否则还比较麻烦，方差不是负数。
                t_kl = gaussian_KL_loss(t_mus, t_logvars)  #这个似乎是，一般的VAEloss?对于t空间的VAE损失
                total_loss += args.lambda_t * t_kl #0.0001,其他系数都是0.001；因此学习率是0.001似乎也合适，不然还得小。。
                t_kl = t_kl.item()
                z_t_kl = compute_two_gaussian_loss(
                        mus, logvars, t_mus, t_logvars)
            ...
            # Generator Backprop.
            total_loss.backward() #genloss+kl_loss+t_kl+z_t_kl，最早是9
            # nn.utils.clip_grad_value_(gen_params,1)
            gen_optimizer.step()
            ...
            if not args.no_answer_recon or not args.no_image_recon:
                ...
                if not args.no_answer_recon:
                    ...
                    recon_answer_loss = recon_a_loss.item()  #最早，2.45左右。

                # Image reconstruction loss.
                if not args.no_image_recon:
                    recon_i_loss = l2_criterion(recon_image_features, image_targets)
                    total_info_loss += args.lambda_i * recon_i_loss #最早，3.55。
                    recon_image_loss = recon_i_loss.item()

                # Info backprop.
                total_info_loss.backward() #最早仅有0.006啊
                # nn.utils.clip_grad_value_(info_params,1)
                info_optimizer.step()
        ...
        # Evaluation and learning rate updates.每个epoch都测试
        run_eval(vqg, val_data_loader, criterion, l2_criterion,
                 args, epoch, scheduler, info_scheduler)
...
if __name__ == '__main__':
    ...
    # Session parameters.
    parser.add_argument('--model-path', type=str, default='weights/tf1_noclip+Adam/',
    ...
    parser.add_argument('--eval-every-n-steps', type=int, default=600,
    ...
    parser.add_argument('--lambda-z-t', type=float, default=0.005,
